(function($) {
  ...
  ActionsBuilder.prototype = {
    init: function() {
      ...
      this.fields.unshift({label: "Choose an Action...", value: ""});
    ...
    buildAction: function(data) {
      ...
      for(var i=0; i < this.fields.length; i++) {
        ...
        var option = $("<option>", {"text": possibleField.label, "value": possibleField.value});
    ...
    buildField: function(field) {
      ...
      if(field.fieldType == "select") {
        var label = $("<label>", {"text": field.label});
        var select = $("<select>", {"name": field.value});

        for(var i=0; i < field.options.length; i++) {
          var optionData = field.options[i];
          var option = $("<option>", {"text": optionData.label, "value": optionData.value});
        ...
        var input = $("<input>", {"type": "text", "name": field.value});
        div.append(input);
      }
      else if(field.fieldType == "textarea") {
        var id = "textarea-" + Math.floor(Math.random() * 100000);
        var area = $("<textarea>", {"name": field.value, "id": id});
        div.append(area);
      }
    ...
    _findField: function(fieldValue) {
      for(var i=0; i < this.fields.length; i++) {
        var field = this.fields[i];
        if(field.value == fieldValue) return field;
      }
    }
  };
