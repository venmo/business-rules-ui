(function($) {
  ...
  ConditionsBuilder.prototype = {
    init: function() {
      ...
      //this.data = this.options.data || {"all": [{field: "", operator: "", value: ""}]};
    ...
    buildConditional: function(ruleData) {
      ...
      addRuleLink.click(function(e) {
        ...
        var newField = {field: f.value, operator: f.operators[0], value: null};
      ...
      addConditionLink.click(function(e) {
        e.preventDefault();
        var f = _this.fields[0];
        var newField = {"all": [{field: f.value, operator: f.operators[0], value: null}]};
    ...
    buildRule: function(ruleData) {
      ...
      for(var i=0; i < this.fields.length; i++) {
        ...
        var option = $("<option>", {
          text: field.label, 
          value: field.value, 
          selected: ruleData.field == field.value
        });
      ...
      fieldSelect.change(function(e) {
        ...
        for(var i=0; i < operators.length; i++) {
          ...
          var option = $("<option>", {
            text: operator.label || operator.value, 
            value: operator.value, 
            selected: ruleData.operator == operator.value
          });
    ...
    operatorsFor: function(fieldValue) {
      for(var i=0; i < this.fields.length; i++) {
        var field = this.fields[i];
